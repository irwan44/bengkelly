...
class _BottomSheetLocationState extends State<BottomSheetLocation> {
  ...
  Future<void> _addMarker(tmpLat, tmpLng) async {
    ...
    // // final String markerIdVal1 = 'marker_id_${_markerIdCounter}_1';
    // // _markerIdCounter++;
    // final MarkerId markerId = MarkerId(markerIdVal);
    // final MarkerId markerId1 = MarkerId(markerIdVal1);
    //
    // final Uint8List markerIcon =
    // await getBytesFromAsset('assets/img/map-marker.png', 140);
    //
    // var orderNumber = widget.data.externalNumber != null &&
    //     widget.data.externalNumber.length > 0
    //     ? widget.data.externalNumber
    //     : widget.data.internalNumber;
    // var driverName =
    //     widget.data.trucks[0]?.assignments[0]?.drivers[0]?.name ?? "N/A";
    // var addressReceiver = widget.data.location.endAddress;
    // creating a new MARKER
    // marker = Marker(
    //   markerId: markerId,
    //   icon: BitmapDescriptor.fromBytes(markerIcon),
    //   position: LatLng(tmpLat, tmpLng),
    //   infoWindow: InfoWindow(title: orderNumber, snippet: driverName),
    // );
    //
    // // creating a new MARKER
    // // marker1 = Marker(
    ...
    // //     infoWindow: InfoWindow(title: addressReceiver));
    //
    // setState(() {
    ...
    //     double? lat = double.tryParse(getLokasi.data![i].geometry?.location?.lat ?? '0.0');
    //     double? lng = double.tryParse(getLokasi.data![i].geometry?.location?.lng ?? '0.0');
    //
    //     if (lat != null && lng != null) {
    //       _markers.add(
    //         Marker(
    //           markerId: MarkerId('marker_$i'), // Gunakan ID yang unik untuk setiap penanda
